...
{{ page.org }} will soon be accepting applications for a GS-15 Fraud Technical Expert. This role is crucial to preserving user privacy. This is an opportunity to make a significant impact by helping other federal agencies securely provide and deliver services to the American public. 
...
{% if page.state != 'upcoming' %}
  {{ page.org }} is hiring for a {{ page.gs_level }} - {{ page.role_name }}. This page contains information related to the role as well as a link to submit your application. Check out [Join TTS Hiring Process]({{site.baseurl}}/hiring-process/) to learn more about the application process.
{% endif %}
